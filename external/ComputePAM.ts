/** PAM样式运算 */
namespace TwinStar.Script.ComputePAM {

	export function execute(
	): void {
		Console.notify('i', `请输入需要计算的PAM文件路径`, []);
		let raw_file = Console.string(null);
...
namespace TwinStar.Support.PopCapAnimation.Convert.Compute {
	...
	function compute_sprite_slot(
		raw_frame: Array<Core.Tool.PopCap.PAM.Manifest.JS_N.Frame>,
		sub_sprite: Array<[SpriteSlot, LayerImageMap]>,
		image: Array<Core.Tool.PopCap.PAM.Manifest.JS_N.Image>,
	...
	function compute_sprite_frame_label(
		frame: Array<Core.Tool.PopCap.PAM.Manifest.JS_N.Frame>,
	...
	export function compute(
		...
		let image: Array<Information.Image> = animation.image.map((e) => ({
			...
			size: e.size!,
			transform: e.transform as Transform,
		}));
	...
	export function compute_fs(
		...
		CoreX.JSON.write_fs_js(ripe_file, ripe, true, true);
		return;
	}
